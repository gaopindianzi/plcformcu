...
#define   CMD_SET_IO_OUT_POWERDOWN_HOLD  44  //0x2C
#define   CMD_GET_IO_OUT_POWERDOWN_HOLD  45  //0x2D
//新增定时器接口
...
{
  uint8_t  cmd;        //cmd,
  uint16_t cmd_index;  //00,00
  uint8_t  cmd_option; //00,
#if 0
  uint8_t  pad;
  uint8_t  cmd_len;
#else
...
//读写寄存器的通用接口
typedef struct _CmdRegisterHead
{
  uint8_t  reg_addr_hi;
  uint8_t  reg_addr_lo;
  uint8_t  reg_len_hi;
